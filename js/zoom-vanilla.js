+function () { "use strict";
  ...
  var vanillaZoom = (function () {
    ...
    function zoomOriginal() {
      ...
      let footerStr = ``
      const showText = targetImage.alt || targetImage.title
      const dataLink = targetImage.getAttribute('data-link')
      if(showText) {
        footerStr += `<span class="viewerTitle">${showText}</span>`
      }
      if(dataLink) {
        footerStr += `<a href="${dataLink}" class="viewerTitleLink" target="_blank">link<svg xmlns="http://www.w3.org/2000/svg" height="48" viewBox="0 96 960 960" width="48" fill="currentColor"><path d="m248 810-42-42 412-412H240v-60h480v480h-60V398L248 810Z"/></svg></a>`
      }
      if(footerStr) {
        const footerEl = document.createElement('div')
        footerEl.className = 'viewerContainer'
